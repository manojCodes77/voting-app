...
router.get('/all',jwtAuthMiddleware, async (req, res) => {
    try {
        ...
        // Return the list of candidates with their vote count
...
router.post('/create', jwtAuthMiddleware, async (req, res) => {
    try {
        ...
        const existingCandidate = await Candidate.findOne({ name: data.name });
        ...
        res.status(200).json({savedCandidate: savedCandidate });
    }
    catch (err) {
        console.log(err);
        res.status(500).json({ error: 'Internal Server Error' });
